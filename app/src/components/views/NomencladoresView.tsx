...
export function NomencladoresView() {
  ...
  const fetchNomencladores = async () => {
    setLoading(true)
    try {
      const res = await fetch('/api/nomencladores', { cache: 'no-store' })
      if (!res.ok) {
        toast.error('Error al cargar nomencladores')
        return
      }
...
function ValoresDialog({
  ...
  const fetchValores = async () => {
    setLoading(true)
    try {
      const res = await fetch(`/api/nomencladores/${nomenclador.codigo}/valores`, { cache: 'no-store' })
      if (!res.ok) {
...
function ValorForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true)
    try {
      const url = `/api/nomencladores/${nomenclador.codigo}/valores`
      const method = 'POST'
      const res = await fetch(url, {
        method,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ codigo, descripcion, ordenVisualizacion: parseInt(orden) }),
      })
      const data = await res.json()
      if (!res.ok) {
