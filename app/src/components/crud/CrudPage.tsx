...
export function CrudPage<T extends { id: string }>({ config }: { config: CrudConfig<T> }) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/${config.resource}?${params.toString()}`, { cache: 'no-store' })
      if (!res.ok) {
        const data = await res.json()
        toast.error('Error al cargar', { description: data.error || `HTTP ${res.status}` })
        return
      }
  ...
  const handleDelete = async (row: T) => {
    ...
    try {
      const res = await fetch(`/api/${config.resource}/${row.id}`, { method: 'DELETE' })
      if (!res.ok) {
        const data = await res.json()
...
function CrudFormModal<T extends { id: string }>({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(
        row
          ? `${config.resourceLabel} actualizado correctamente`
          : `${config.resourceLabel} creado correctamente`
      )
      onSaved()
    } catch (e: any) {
      toast.error('Error de conexión', { description: e.message })
