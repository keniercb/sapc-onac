...
export function Topbar({ onToggleSidebar }: { onToggleSidebar: () => void }) {
  ...
  const loadNotifs = useCallback(async () => {
    try {
      const res = await fetch('/api/notificaciones?limit=20', { cache: 'no-store' })
      if (!res.ok) return
  ...
  const handleMarcarLeida = async (destinatarioId: string) => {
    try {
      await fetch('/api/notificaciones', {
        ...
        body: JSON.stringify({ accion: 'marcar_leida', destinatarioId }),
      })
      loadNotifs()
    } catch {
    ...
    try {
      ...
      if (!res.ok) {
        if (data.detalles && Array.isArray(data.detalles)) {
          toast.error('Política de complejidad', { description: data.detalles.join(', ') })
        } else {
      ...
      onOpenChange(false)
      // Cerrar sesión porque se invalidaron las demás sesiones (incluida la actual si el backend decide)
      // En Fase 0, nuestra implementación mantiene la sesión actual activa.
    } catch (err: any) {
