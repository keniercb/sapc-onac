...
|---|---|---|---|
| Runtime Frontend (build) | **Node.js** | 26.2.8 LTS | Mismo runtime que backend para homogeneidad del monorepo. |
...
| Componente | Tecnología | Versión | Justificación |
|---|---|---|---|
| Runtime | **Node.js** | 26.2.8 LTS | Versión requerida por el cliente; soporte extendido, performance estable. |
...
- **Node.js 26.2.8 LTS** (versión requerida por el cliente). Pin en `.nvmrc` y en `Dockerfile` base. Mantener dentro del ciclo de parches de la rama 26.x.
- PostgreSQL 16 hasta noviembre 2028.
- Next.js 14 (estable) → evaluación de Next.js 15 LTS cuando se estabilice.

> **Pin estricto de Node.js 26.2.8:** el archivo `.nvmrc` debe contener exactamente `26.2.8` (no `26` ni `lts/*`), y la imagen base Docker debe ser `node:26.2.8-bookworm-slim` (o `alpine` si se prefiere menor tamaño). Cualquier actualización de parche requiere validación explícita del equipo de arquitectura y debe quedar registrada como ADR.
...
2. Setup **Node.js 26.2.8** exacto (validar versión antes de continuar el pipeline).
3. Setup pnpm 9.
...
|---|---|---|---|
| Baja conectividad internacional (npm/Docker Hub) | Alta | Alto | Mirror local de npm (Verdaccio/Proxy Nexus) y registry Docker interno; precarga de imágenes base. Pin estricto de Node.js 26.2.8 pre-cacheado en el runner. |
